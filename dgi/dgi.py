...
class DGI(nn.Module):
    def __init__(self, in_feats, n_hidden, n_layers, activation, dropout, l2_mutual=False):
    ...
    def forward(self, features, gs):
        ...
        if len(features) == 2:
            # Multigraph
            l = self._self_mutual_info(features[0], gs[0])\
                    + self._self_mutual_info(features[1], gs[1])
